...
const handleBook = async () => {
  try {
    // alert('Successfully booked');  // Or inform user they're already joined

    const res = await axios.get(`<span class="math-inline">\{apiUri\}/rides/</span>{rideId}/join`, { withCredentials: true });
    console.log(res);
    // Check for successful booking or existing membership
    if ( res.status===200) {
      alert(`Ride booked successfully!`);  // Or inform user they're already joined
    } else {
      console.error('Unexpected response:', res.data);
      toast.error('An unexpected error occurred while booking the ride.');
    }
  } catch (err) {
    console.error(err);
    toast.error(err.response?.data || "An error occurred while booking the ride.");
  }
};
